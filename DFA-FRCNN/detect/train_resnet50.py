...
def create_model(num_classes):
    ...
    # net = MobileNetV2()
    net = resnet50()
    weights_dict = torch.load("weights/resnet50-pre.pth")
    net.load_state_dict(weights_dict, strict=False)
    backbone = []
    backbone.append(net.conv1)
    backbone.append(net.bn1)
    backbone.append(net.relu)
    backbone.append(net.maxpool)
    backbone.append(net.layer1)
    backbone.append(net.layer2)
    backbone.append(net.layer3)
    backbone.append(net.layer4)
    backbone = nn.Sequential(*backbone)
    backbone.out_channels = 2048  # 设置对应backbone输出特征矩阵的channels
    ...
    for epoch in range(num_epochs):
        ...
        if epoch in range(num_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/res50-model-{}.pth".format(epoch))
