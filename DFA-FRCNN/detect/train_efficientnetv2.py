...
from backbone import EfficientNetV2, effv2_fpn_backbone



def create_model(num_classes):
    backbone = effv2_fpn_backbone(trainable_layers=4)
    ...
    # backbone.out_channels = 256  # 设置对应backbone输出特征矩阵的channels
    #
    # anchor_generator = AnchorsGenerator(sizes=((32, 64, 128, 256, 512),),
    #                                     aspect_ratios=((0.5, 1.0, 2.0),))
    #
    # roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],  #
    #                                                 output_size=[7, 7],   # roi_pooling
    #                                                 sampling_ratio=2)
    #
    ...
    #
    # dict1 = torch.load("weights/fasterrcnn_resnet50_fpn.pth")
    ...
    #     print(key, value)
    # dict = torch.load("weights/pre_efficientnetv2-s.pth")

    # for key,value in dict2.items():
    #     if key[:3] == "rpn" or key[:3] == "roi":
    #         value2 = dict1[key]
    #         if value.size() == value2.size():
    #             dict2[key] = dict1[key]

    # torch.save(dict2,"./weights/effv2.pth")
