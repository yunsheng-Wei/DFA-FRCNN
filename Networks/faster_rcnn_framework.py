...
from backbone import EfficientNetV2
...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        # 下面这几行代码只使用于Efficient，使用其他backbone时请将这行代码注销
        ##############################################################
        # net = EfficientNetV2()
        # pre_weights = torch.load("weights/pre_efficientnetv2-m.pth")
        # net.load_state_dict(pre_weights)
        # first_layer = net.stem
        # # first_layer.
        # pre_features = first_layer(images.tensors)
        # features = self.backbone(pre_features)  # 将图像输入backbone得到特征图
        ###############################################################
